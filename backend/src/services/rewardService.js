...
async function plantTree(citizen, issue) {
  ...
  const treeData = {
    ...
    project: 'CityZen Green India Initiative',
...
async function generateCertificate(citizen, issue) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(11).fillColor('#333').font('Helvetica') 
      .text('Awarded under the CityZen National Civic Participation Initiative', { align: 'center' }); 
    ...
    doc.fontSize(9).fillColor('#666').font('Helvetica') 
      .text('CityZen — Building Better Communities Together', mx, h - mx - 30, { align: 'center' }); 
